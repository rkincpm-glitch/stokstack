...
export default function ManageCategoriesPage() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      const { data, error } = await supabase.from("locations").select("id,name,user_id").order("name");
  ...
  const add = async () => {
    ...
    const res = await safeInsert("locations", payload);
    if (res.error) {
  ...
  const update = async (id: string, name: string) => {
    setError(null);
    setSavingId(id);
    const res = await safeUpdate("locations", id, { name: name.trim() });
  ...
  if (loading) {
    return <div className="min-h-screen flex items-center justify-center text-slate-500">Loading locations…</div>;
  }
    ...
    <div className="min-h-screen bg-slate-50">
      <header className="bg-white border-b">
        <div className="max-w-3xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <h1 className="text-sm font-semibold text-slate-900">Manage Locations</h1>
        </div>
      </header>
      ...
      <main className="max-w-3xl mx-auto px-4 py-6 space-y-4">
        ...
        <div className="bg-white border rounded-2xl overflow-hidden">
          ...
          <div className="divide-y">
            ...
            {sorted.length === 0 && (
              <div className="px-4 py-6 text-sm text-slate-500">No locations yet.</div>
            )}
          </div>
        </div>
      </main>
